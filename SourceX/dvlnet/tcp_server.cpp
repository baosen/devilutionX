...
namespace net {
	...
	tcp_server::tcp_server(asio::io_context &ioc, std::string bindaddr, unsigned short port, std::string pw)
	    ...
	    , pktfty(pw)
	{
		acceptor = std::make_unique<asio::ip::tcp::acceptor>(ioc, asio::ip::tcp::endpoint(asio::ip::address::from_string(bindaddr), port));
		start_accept();
	}

	std::string tcp_server::localhost_self()
	{
		const auto addr = acceptor->local_endpoint().address();
	...
	void tcp_server::handle_recv(scc con, const asio::error_code &ec, size_t bytes_read)
	{
		if (ec || bytes_read == 0) {
			drop_connection(con);
			return;
