...
namespace net {
	...
	void packet_in::decrypt()
	{
		...
		if (!disable_encryption) {
			if (encrypted_buffer.size() < crypto_secretbox_NONCEBYTES + crypto_secretbox_MACBYTES + sizeof(packet_type) + 2 * sizeof(plr_t))
				throw packet_exception();
			decrypted_buffer.resize((encrypted_buffer.size() - crypto_secretbox_NONCEBYTES - crypto_secretbox_MACBYTES));
			if (crypto_secretbox_open_easy(decrypted_buffer.data(), encrypted_buffer.data() + crypto_secretbox_NONCEBYTES, encrypted_buffer.size() - crypto_secretbox_NONCEBYTES, encrypted_buffer.data(), key.data()))
				throw packet_exception();
		} else {
	...
	void packet_out::encrypt()
	{
		...
		process_data();
		if (!disable_encryption) {
			const auto len_cleartext = encrypted_buffer.size();
			...
			if (crypto_secretbox_easy(encrypted_buffer.data() + crypto_secretbox_NONCEBYTES, encrypted_buffer.data() + crypto_secretbox_NONCEBYTES, len_cleartext, encrypted_buffer.data(), key.data()))
				ABORT();
		}
		have_encrypted = true;
	}
	...
	packet_factory::packet_factory(std::string pw)
	{
		...
		if (crypto_pwhash(key.data(), crypto_secretbox_KEYBYTES, pw.data(), pw.size(), reinterpret_cast<const unsigned char *>(salt.data()), crypto_pwhash_argon2id_OPSLIMIT_INTERACTIVE, crypto_pwhash_argon2id_MEMLIMIT_INTERACTIVE, crypto_pwhash_ALG_ARGON2ID13))
			ABORT();
	}
